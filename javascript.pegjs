...
// Modifications include whitespace in source tree

{
  ...
  function unescapeCharSequence(string) {
    return
      string
      .replace(/[\\\b\f\n\r\t\v]/g, function(m) {
        switch(m) {
  ...
  var unparseArrayLiteral = [
    "open", "wsAfterOpening",
    {name: "elements", map: function(els, unparser) {
        return els.map(function(el) { return el ? unparser(el) : "" }).join(",") } },
    "wsBeforeClosing", "close"]
}
...
NullLiteral
  = NullToken { return { type: "Literal", value: null, unparse: ["value"] }; }

BooleanLiteral
  = TrueToken  { return { type: "Literal", value: true, unparse: ["value"]  }; }
  / FalseToken { return { type: "Literal", value: false, unparse: ["value"] }; }
...
DecimalLiteral
  = DecimalIntegerLiteral "." DecimalDigit* ExponentPart? {
      return { type: "Literal", value: parseFloat(text()), unparse: ["value"] };
    }
...
HexIntegerLiteral
  = prefix:"0x"i zeroes:$[0]* digits:$HexDigit+ {
      return { type: "Literal", value: parseInt(digits, 16), prefix: prefix, zeroes: zeroes,
               unparse: ["prefix", "zeroes", {name: "value", map: function(v) { return v.toString(16);} }] };
     }
...
StringLiteral "string"
  = delimiter:'"' chars:DoubleStringCharacter* '"' {
      return { type: "Literal", delimiter: delimiter,
               value: chars.join(""),
               unparse: ["delimiter", {name: "value", map: function(v) { return unescapeCharSequence(v).replace(/"/g, '\\"')}}, "delimiter"] };
    }
  / delimiter:"'" chars:SingleStringCharacter* "'" {
      return { type: "Literal", delimiter: delimiter,
               value: chars.join(""),
               unparse: ["delimiter", {name: "value", map: function(v) { return unescapeCharSequence(v).replace(/'/g, "\\'")}}, "delimiter"] };
    }
...
RegularExpressionLiteral "regular expression"
  = delimiter:"/" pattern:$RegularExpressionBody "/" flags:$RegularExpressionFlags {
      var value;
      ...
      return { type: "Literal", value: value, delimiter: delimiter,
        unparse: ["delimiter", {name:"value", map: function(v) { return v.source }}, "delimiter", {name:"value", map: function(v) { return v.flags;}}] };
    }
...
BreakToken      = token:"break"      !IdentifierPart { return token; }
CaseToken       = token:"case"       !IdentifierPart { return token; }
CatchToken      = token:"catch"      !IdentifierPart { return token; }
ClassToken      = token:"class"      !IdentifierPart { return token; }
ConstToken      = token:"const"      !IdentifierPart { return token; }
ContinueToken   = token:"continue"   !IdentifierPart { return token; }
DebuggerToken   = token:"debugger"   !IdentifierPart { return token; }
DefaultToken    = token:"default"    !IdentifierPart { return token; }
DeleteToken     = token:"delete"     !IdentifierPart { return token; }
DoToken         = token:"do"         !IdentifierPart { return token; }
ElseToken       = token:"else"       !IdentifierPart { return token; }
EnumToken       = token:"enum"       !IdentifierPart { return token; }
ExportToken     = token:"export"     !IdentifierPart { return token; }
ExtendsToken    = token:"extends"    !IdentifierPart { return token; }
FalseToken      = token:"false"      !IdentifierPart { return token; }
FinallyToken    = token:"finally"    !IdentifierPart { return token; }
ForToken        = token:"for"        !IdentifierPart { return token; }
FunctionToken   = token:"function"   !IdentifierPart { return token; }
GetToken        = token:"get"        !IdentifierPart { return token; }
IfToken         = token:"if"         !IdentifierPart { return token; }
ImportToken     = token:"import"     !IdentifierPart { return token; }
InstanceofToken = token:"instanceof" !IdentifierPart { return token; }
InToken         = token:"in"         !IdentifierPart { return token; }
NewToken        = token:"new"        !IdentifierPart { return token; }
NullToken       = token:"null"       !IdentifierPart { return token; }
ReturnToken     = token:"return"     !IdentifierPart { return token; }
SetToken        = token:"set"        !IdentifierPart { return token; }
SuperToken      = token:"super"      !IdentifierPart { return token; }
SwitchToken     = token:"switch"     !IdentifierPart { return token; }
ThisToken       = token:"this"       !IdentifierPart { return token; }
ThrowToken      = token:"throw"      !IdentifierPart { return token; }
TrueToken       = token:"true"       !IdentifierPart { return token; }
TryToken        = token:"try"        !IdentifierPart { return token; }
TypeofToken     = token:"typeof"     !IdentifierPart { return token; }
VarToken        = token:"var"        !IdentifierPart { return token; }
VoidToken       = token:"void"       !IdentifierPart { return token; }
WhileToken      = token:"while"      !IdentifierPart { return token; }
WithToken       = token:"with"       !IdentifierPart { return token; }
...
PrimaryExpression
  = token:ThisToken { return { type: "ThisExpression", token:token, unparse:["token"] }; }
  / Identifier
  / Literal
  / ArrayLiteral
  / ObjectLiteral
...
ArrayLiteral
  = open:"[" ws0:__ elision:(Elision __)? close:"]" {
      return {
        type: "ArrayExpression",
        open: open,
        wsAfterOpening: ws0,
        elements: optionalList(extractOptional(elision, 0)),
        wsBeforeClosing: extractOptional(elision, 1) || "",
        close: close,
        unparse: unparseArrayLiteral
      };
    }
  / open:"[" ws0:__ elements:ElementList ws1:__ close:"]" {
      return {
        type: "ArrayExpression",
        open: open,
        wsAfterOpening: ws0,
  ...
  / open:"[" ws0:__ elements:ElementList ws1:__ "," ws2:__ elision:(Elision __)? close:"]" {
      return {
        type: "ArrayExpression",
        open: open,
        wsAfterOpening: ws0,
...
BreakStatement
  = token:BreakToken wsAfter:EOS {
      return { type: "BreakStatement", token: token, label: null, wsAfter: wsAfter, unparse: ["token", "wsAfter"] };
    }
